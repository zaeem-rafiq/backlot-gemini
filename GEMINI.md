...
1. **Antigravity Pinning:** Antigravity must remain pinned to Google Gemini models (`gemini-3.8-flash`, `gemini-3.5-flash`, `gemini-3-flash-preview`, `gemini-2.5-flash`) at all times. Selecting non-Google models violates hackathon rules (§7.B).
...
- **Partner Integration:** Parallel Search API (Direct authenticated REST client via `fetch`) called at runtime in Marquee for market intelligence & festival signals
