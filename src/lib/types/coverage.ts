...
export const CoverageSchema = z.object({
  ...
  genre: z.array(z.string()).min(1).max(3).describe("Primary and sub-genres (up to 3)"),
  tone: z.string().describe("Tone descriptor, e.g., 'Atmospheric neo-noir with slow-burn dread'"),
  themes: z.array(z.string()).min(1).max(4).describe("Core thematic motifs (up to 4)"),
  comparables: z.array(ComparableSchema).min(1).max(3).describe("1-3 market comparables with rationale"),
  strengths: z.array(z.string()).min(2).max(5).describe("2-5 standout artistic or commercial strengths"),
  concerns: z.array(z.string()).min(1).max(5).describe("1-5 candid constructive production/narrative risks"),
